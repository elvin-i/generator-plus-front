      ...
      >
        ...
        <span slot="action" slot-scope="text, record">
          <template>
            ...
            <a @click="handleGen(record.id,record.url)">生成</a>
...
import Gen from '@/views/frame/archetypes/Gen'
import AddOrEditForm from '@/views/frame/archetypes/addOrEditForm'
import InfoTable from '@/views/frame/archetypes/infoTable'
...
export default {
  ...
  methods: {
    ...
    handleGen (id, url) {
      ...
      const commonRequest = this.commonRequest
      request({
        url: '/archetypes/getLastedLogById',
        method: 'post',
        dataType: 'json',
        data: commonRequest
      }).then(res => {
        this.genvalue = { 'archetypesId': id, 'archetypeUrl': url }
        if (res.head.status === 'S') {
          this.genvalue.artifactId = res.body.artifactId
          this.genvalue.groupId = res.body.groupId
          this.genvalue.version = res.body.version
          this.genvalue.basePackage = res.body.basePackage
        }
        this.$refs.Gen.show()
      }).catch(() => {
        this.genvalue = { 'archetypesId': id, 'archetypeUrl': url }
